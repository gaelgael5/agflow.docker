...
async def execute_step(deployment_id: UUID) -> None:
    ...
    try:
        ...
        current_env = merge_env_with_values(
            deployment.generated_env or "",
            {k: str(v) for k, v in (deployment.accumulated_env or {}).items()},
        )
        ...
        parsed_json = parse_last_json(result["stdout"])
        ...
        with contextlib.suppress(Exception):
            await log_bus.publish(deployment_id, {"type": "internal_error", "error": str(exc)})
    finally:
